...
import { 
  faTimes, faBolt, faPercent, faPlus
} from '@fortawesome/free-solid-svg-icons';
import SprayLoader from '../common/SprayLoader';
import { insertLimitedOffer, GetLimitedOffers } from '../../APIS/apis/LimitedOfferApi';
import { GetProduct } from '../../APIS/apis/ProductApi';
import { useDispatch, useSelector } from 'react-redux';
import { toast } from "react-toastify";

export default function AddLimitedOfferModal({ isOpen, onClose }) {
  ...
  const onSubmit = async (data) => {
    ...
    try {
      ...
      if (res?.payload?.status === "success") {
        ...
        toast.error(res?.payload?.message || "Failed to create offer");
      }
    } catch (error) {
        ...
        <div style={{
          ...
          <div>
            ...
            <p style={{ margin: '0.25rem 0 0 0', color: '#6c757d', fontSize: '0.875rem' }}>
              Set up a new flash deal or limited time offer
            </p>
          </div>
          <button
            onClick={handleClose}

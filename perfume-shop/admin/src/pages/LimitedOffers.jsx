...
import { 
  faBolt, faFire, faEye, faEdit, faTrash, faPlus, faFilter, faSearch, faCalendarAlt, faStar
} from '@fortawesome/free-solid-svg-icons';
import AddLimitedOfferModal from '../components/modals/AddLimitedOfferModal';
import ViewLimitedOfferModal from '../components/modals/ViewLimitedOfferModal';
import EditLimitedOfferModal from '../components/modals/EditLimitedOfferModal';
import DeleteConfirmationModal from '../components/modals/DeleteConfirmationModal';
import { GetLimitedOffers, DeleteLimitedOffer } from '../APIS/apis/LimitedOfferApi';
...
export default function LimitedOffers() {
  ...
  const [viewModal, setViewModal] = useState({ isOpen: false, offer: null });
  const [editModal, setEditModal] = useState({ isOpen: false, offer: null });
  ...
  const { limitedOfferData } = useSelector((state) => state?.LimitedOfferSlice);
  ...
  const handleViewClick = (offer) => {
    setViewModal({ isOpen: true, offer });
  };

  // Handle edit
  const handleEditClick = (offer) => {
    setEditModal({ isOpen: true, offer });
  };
  ...
  const handleDeleteConfirm = async () => {
    ...
    try {
      ...
      if (res?.payload?.status === 'success' || res?.payload?.success === true) {
        ...
        toast.error(res?.payload?.message || 'Failed to delete offer');
      }
    } catch (error) {
  ...
  const filteredOffers = offers.filter(offer => {
    const statusMatch = !selectedStatus || selectedStatus.value === 'all' || offer.status.toLowerCase() === selectedStatus.value;
  ...
  return (
    <div>
      ...
      <div className="card border-0 shadow-sm" style={{
        ...
        {/* Offers Grid - 4 cards per row matching Sales */}
      ...
      {/* Modals */}
      <AddLimitedOfferModal 
        isOpen={isAddModalOpen} 
        onClose={() => setIsAddModalOpen(false)} 
      />
      <ViewLimitedOfferModal 
        isOpen={viewModal.isOpen} 
        onClose={() => setViewModal({ isOpen: false, offer: null })} 
        offer={viewModal.offer}
        onEdit={(offer) => {
          setViewModal({ isOpen: false, offer: null });
          setEditModal({ isOpen: true, offer });
        }}
      />
      <EditLimitedOfferModal 
        isOpen={editModal.isOpen} 
        onClose={() => setEditModal({ isOpen: false, offer: null })} 
        offer={editModal.offer}
      />
      <DeleteConfirmationModal
        isOpen={deleteModal.isOpen}
        onClose={() => setDeleteModal({ isOpen: false, offer: null })}
        onConfirm={handleDeleteConfirm}
        itemName={deleteModal.offer?.title}
